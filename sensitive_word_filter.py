...
def get_hierarchical_layout(G, automaton):
    ...
    for level, nodes in nodes_by_level.items():
        ...
        if num_nodes == 1:
            ...
            total_width = max(15.0, num_nodes * 2.5)
...
def get_best_layout(G, automaton, num_nodes):
    ...
    pos = nx.spring_layout(G, k=10/math.sqrt(num_nodes), iterations=200, seed=42, scale=3.5)
    return pos, 5

def create_interactive_graph(G, active_states, active_goto_edges, active_fail_edges, automaton, num_nodes):
...
with col2:
    ...
    if st.session_state.automaton and st.session_state.sensitive_words:
        ...
        with col_info1:
            st.caption(f"📊 {num_nodes} states, {metrics['num_goto_edges']} goto edges")
        with col_info2:
            st.caption("💡 Hierarchical layout")
        ...
        with tab1:
            ...
            fig, config = create_interactive_graph(
                ...
                st.session_state.automaton, num_nodes
            )

            st.plotly_chart(fig, width='stretch', config=config)
            st.caption("🟠 Active path | 🟢 Match states | 🔵 Goto transitions | 🟡 Active states")
        ...
        with tab2:
            ...
            st.caption("δ(char) = Goto function | Fail = Failure function | Output = Matched words")
